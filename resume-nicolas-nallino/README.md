...
```bash
resume-nicolas-nallino$ sam build --use-container
```
...
```bash
resume-nicolas-nallino$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
resume-nicolas-nallino$ sam local start-api
resume-nicolas-nallino$ curl http://localhost:3000/
```
...
```bash
resume-nicolas-nallino$ sam logs -n HelloWorldFunction --stack-name resume-nicolas-nallino --tail
```
...
```bash
resume-nicolas-nallino$ pip install -r tests/requirements.txt --user
# unit test
resume-nicolas-nallino$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
resume-nicolas-nallino$ AWS_SAM_STACK_NAME="resume-nicolas-nallino" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "resume-nicolas-nallino"
```
